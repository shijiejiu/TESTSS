...
import { ChainId, Currency, currencyEquals, ETHER, Percent, WETH } from 'swap-supproted-multichain-sdk'
...
export default function RemoveLiquidity({
  ...
  async function onAttemptToApprove() {
    ...
    const domain = {
      name: 
      chainId === ChainId.MAINNET ? 'Uniswap V2' :
      chainId === ChainId.BSCTEST ? 'Pancake LPs' :
      chainId === ChainId.BSC ? 'Pancake LPs' :
      chainId === ChainId.SEPOLIA ? 'Pancake LPs' : 'Uniswap V2',
    ...
    return (
      <AutoColumn gap={'md'} style={{ marginTop: '20px' }}>
        ...
        <TYPE.italic fontSize={12} color={theme.text2} textAlign="left" padding={'12px 0 0 0'}>
          {`输出是预计的。如果价格变化超过 ${allowedSlippage /
            100}% 您的交易将会回退.`}
        </TYPE.italic>
      </AutoColumn>
    )
      ...
      <>
        <RowBetween>
          <Text color={theme.text2} fontWeight={500} fontSize={16}>
            {'UNI ' + currencyA?.symbol + '/' + currencyB?.symbol} 销毁
          </Text>
          <RowFixed>
  ...
  const oneCurrencyIsETH = currencyA === ETHER[chainId??ChainId.MAINNET] || currencyB === ETHER[chainId??ChainId.MAINNET]
            ...
            <BlueCard>
              <AutoColumn gap="10px">
                <TYPE.link fontWeight={400} color={'primaryText1'}>
                  <b>提示:</b> 移除池子代币会将您的头寸按当前汇率转换回基础代币，比例与您在池中的份额相关。累计的费用已包含在您收到的金额中。
                </TYPE.link>
              </AutoColumn>
            </BlueCard>
            <LightCard>
