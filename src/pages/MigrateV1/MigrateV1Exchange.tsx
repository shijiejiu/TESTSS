...
// import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WETH } from 'swap-supproted-multichain-sdk'
import { ETHER, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WETH, ChainId } from 'swap-supproted-multichain-sdk'
    ...
    <>
      ...
      <RowBetween mb="1rem">
        ...
        <RowFixed>
          ...
          <CurrencyLogo size="20px" style={{ marginLeft: '8px' }} currency={ETHER[chainId??ChainId.MAINNET]} />
        </RowFixed>
      </RowBetween>
    </>
  )
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const ethWorth: CurrencyAmount = exchangeETHBalance
    ? CurrencyAmount.ether(exchangeETHBalance.multiply(shareFraction).multiply(WEI_DENOM).quotient, chainId??ChainId.MAINNET)
    : CurrencyAmount.ether(ZERO, chainId??ChainId.MAINNET)
